...
class DQNAgent() :
    def __init__(self, q_func, optimizer, n_act, replay_buffer, batch_size, replay_start_size, epsilon = 0.1, gamma = 0.9) :
    ...
    def learn_batch(self, obs_batch, action_batch, reward_batch, next_obs_batch, done_batch) :
        ...
        targetQ = reward_batch + (1 - done_batch) * self.gamma * self.q_func(next_obs_batch).max(1)[0]
    ...
    # def updateTargetQ(self) :
        # self.targetQ = self.tau * self.Q + (1-self.tau) * self.targetQ
